...
async function loadRecurringAppointments() {
    try {
        ...
        appointments.forEach(appointment => {
            ...
            appointmentElement.className = 'recurring-appointment-item';
        ...
        document.getElementById('recurring-appointments-list').innerHTML = 
            '<p style="color: #e74c3c; text-align: center;">Erro ao carregar horários recorrentes</p>';
    }
}
...
async function createRecurringAppointment() {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.error || 'Erro ao criar horário recorrente');
        }
        ...
        console.error('Erro ao criar horário recorrente:', error);
...
async function deactivateRecurringAppointment(id) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.error || 'Erro ao remover horário recorrente');
        }
        
        alert('✅ Horário recorrente removido com sucesso!');
        await loadRecurringAppointments();
    } catch (error) {
        console.error('Erro ao remover horário recorrente:', error);
        alert(`❌ Erro: ${error.message}`);
    }
}
...
async function generateRecurringAppointments() {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.error || 'Erro ao gerar agendamentos');
        }
...
async function loadRecurringSelects() {
    try {
        const barberResponse = await fetch('/api/barbers', { headers: { 'Accept': 'application/json' } });
        if (!barberResponse.ok) throw new Error('Erro ao carregar barbeiros');
        const barbers = await barberResponse.json();
        ...
        const clientResponse = await fetch('/api/users', { headers: { 'Accept': 'application/json' } });
        if (!clientResponse.ok) throw new Error('Erro ao carregar clientes');
        const clients = await clientResponse.json();
...
// Chame generateRecurringAppointments periodicamente (a cada 24h)
setInterval(generateRecurringAppointments, 24 * 60 * 60 * 1000);
